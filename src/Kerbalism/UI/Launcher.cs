	...
	sealed class Launcher
	{
		...
		internal void On_gui()
		{
			...
			if (vesselListLauncher.toggleButton.Value || vesselListLauncher.IsHovering || (win_rect.width > 0f && win_rect.Contains(Mouse.screenPos)))
			{
				...
				// get window size
				float width = GameLogic.IsEditor() ? Planner.Planner.Width() : monitor.Width();
				float height = GameLogic.IsEditor() ? Planner.Planner.Height() : monitor.Height();
				...
				{
					left -= !GameLogic.IsEditor() ? at_bottom_offset_x : at_bottom_editor_offset_x;
