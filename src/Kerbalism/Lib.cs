	...
	static class Lib
	{
		...
		*/
		...
		internal static bool HasPart(Vessel v, string part_name)
		{
			...
			bool ret = false;
			foreach(string name in String.Tokenize(part_name, ','))
			{
				if (v.loaded)
		...
		internal static PartResource AddResource(Part p, string res_name, double amount, double capacity)
		{
			...
			{
				Logging.Log(String.BuildString("error while adding ", res_name, ": the resource doesn't exist"), Logging.LogLevel.Error);
				return null;
			}
		...
		static void SetResourceCapacity( Part p, string res_name, double capacity )
		{
			...
			if (!p.Resources.Contains( res_name ))
			{
				Logging.Log( String.BuildString( "error while setting capacity for ", res_name, ": the resource is not in the part" ), Logging.LogLevel.Error);
				return;
			}
