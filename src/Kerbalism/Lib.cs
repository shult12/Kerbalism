...
namespace KERBALISM
{
	static class Lib
	{
		...
		internal static UInt64 Clocks()
		{
			return (UInt64)Stopwatch.GetTimestamp();
		}

		///<summary>convert from clocks to microseconds</summary>
		internal static double Microseconds(UInt64 clocks)
		{
			return clocks * 1000000.0 / Stopwatch.Frequency;
		}


		internal static double Milliseconds(UInt64 clocks)
		{
			return clocks * 1000.0 / Stopwatch.Frequency;
		}


		internal static double Seconds(UInt64 clocks)
		{
		...
		internal sealed class ResourceUnitInfo : IConfigNode
		{
...
#pragma warning restore CS0649
		...
		static string SIRate(double rate, ResourceUnitInfo rui, int sigFigs = 3, bool longPrefix = false)
		{
			return SIRate(rate * rui.MultiplierToUnit, rui.RateUnit, sigFigs, longPrefix);
		}

		static string SIAmount(double amount, string unit, int sigFigs = 3, bool longPrefix = false)
		{
			return KSPUtil.PrintSI(amount, unit, sigFigs, longPrefix);
		}

		static string SIAmount(double rate, int resID, int sigFigs = 3, bool longPrefix = false)
		{
			return SIAmount(rate, GetResourceUnitInfo(resID), sigFigs, longPrefix);
		}

		static string SIAmount(double rate, ResourceUnitInfo rui, int sigFigs = 3, bool longPrefix = false)
		{
		...
		static string SIRadiation(double rad, bool nominal = true)
		{
		...
		static string SIPressure(double v)
		{
			v *= 1000d;
		...
		internal static string HumanReadableDuration(double d, bool fullprecison = false)
		{
			if (!fullprecison)
			{
		...
		internal static string HumanReadableCountdown(double duration, bool compact = false)
		{
			return BuildString("T-", HumanReadableDuration(duration, !compact));
		}

		///<summary> Pretty-print a range (range is in meters) </summary>
		internal static string HumanReadableDistance(double distance)
		{
		...
		///<summary> Pretty-print magnetic strength </summary>
		internal static string HumanReadableField(double strength)
		{
			if (Settings.UseSIUnits)
		...
		internal static string HumanReadableRadiation(double rad, bool nominal = true)
		{
			if (Settings.UseSIUnits)
		...
		internal static string HumanReadablePerc(double v, string format = "F0")
		{
			return BuildString((v * 100.0).ToString(format), "%");
		}

		///<summary> Pretty-print pressure (value is in kPa) </summary>
		internal static string HumanReadablePressure(double v)
		{
			if (Settings.UseSIUnits)
				return SIPressure(v);
		...
		internal static string HumanReadableVolume(double v)
		{
			return Lib.BuildString(v.ToString("F2"), " m³");
		}

		///<summary> Pretty-print surface (value is in m^2) </summary>
		internal static string HumanReadableSurface(double v)
		{
			return Lib.BuildString(v.ToString("F2"), " m²");
		}

		///<summary> Pretty-print mass </summary>
		internal static string HumanReadableMass(double v)
		{
		...
		internal static string HumanReadableCost(double v)
		{
		...
		internal static string HumanReadableDataSize(double size)
		{
		...
		internal static string HumanReadableDataRate(double rate)
		{
		...
		internal static string HumanReadableSampleSize(double size)
		{
			return HumanReadableSampleSize(SampleSizeToSlots(size));
		}

		internal static string HumanReadableSampleSize(int slots)
		{
		...
		internal static string HumanReadableScience(double value, bool compact = true)
		{
			if (compact)
		...
		static bool ModuleEnableInScienceAndCareer(PartModule m)
		{
		...
		*/
		...
		/// </summary>
		internal static double PartBoundsVolume(Part p, bool applyCylinderFactor = false)
		{
		...
		/// note: this can only be called when part has not been rotated
		/// </summary>
		internal static double PartBoundsSurface(Part p, bool applyCylinderFactor = false)
		{
			return applyCylinderFactor ? BoundsSurface(GetPartBounds(p)) * 0.95493 : BoundsSurface(GetPartBounds(p));
		}

		internal static double BoundsVolume(Bounds bb)
		{
			Vector3 size = bb.size;
			return size.x * size.y * size.z;
		}

		internal static double BoundsSurface(Bounds bb)
		{
			Vector3 size = bb.size;
			double a = size.x;
			double b = size.y;
		...
		internal static PartVolumeAndSurfaceInfo GetPartVolumeAndSurface(
			Part part,
			bool logAll = false,
